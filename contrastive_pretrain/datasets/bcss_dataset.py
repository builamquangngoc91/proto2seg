...
class BCSSDataset(Dataset):
    ...
    def __init__(
        ...
        if base_dir is None:
            # Use the project root (two levels up from CSV file location)
            # CSV is at data/BCSS_128/BCSS_128.csv, so go up twice to reach project root
            csv_dir = os.path.dirname(os.path.abspath(csv_path))
            self.base_dir = os.path.dirname(csv_dir)  # Go up from data/BCSS_128 to data
            self.base_dir = os.path.dirname(self.base_dir)  # Go up from data to project root
        else:
    ...
    def __getitem__(self, idx: int) -> Tuple:
        ...
        # Load image - resolve path relative to base_dir if needed
        img_path = row['patch_path']
        if not os.path.isabs(img_path) and not os.path.exists(img_path):
            # Strip leading ./ if present
            img_path_clean = img_path[2:] if img_path.startswith('./') else img_path
            # Try relative to base_dir
            img_path = os.path.join(self.base_dir, img_path_clean)
        if not os.path.exists(img_path):
            raise FileNotFoundError(f"Image not found: {img_path}")
        
        image = Image.open(img_path).convert('RGB')
        
        # Load and process mask - resolve path relative to base_dir if needed
        mask_path = row['mask_path']
        if not os.path.isabs(mask_path) and not os.path.exists(mask_path):
            # Strip leading ./ if present
            mask_path_clean = mask_path[2:] if mask_path.startswith('./') else mask_path
            # Try relative to base_dir
            mask_path = os.path.join(self.base_dir, mask_path_clean)
...
if __name__ == '__main__':
    ...
    if os.path.exists('./data/BCSS_128/BCSS_128.csv'):
        csv_path = './data/BCSS_128/BCSS_128.csv'
    elif os.path.exists('../data/BCSS_128/BCSS_128.csv'):
        csv_path = '../data/BCSS_128/BCSS_128.csv'
    else:
        print(f"ERROR: Dataset not found")
        print("Please run tools/prepare_bcss_128.py first")
        print("Make sure to run this script from the project root or datasets directory")
        exit(1)
